...
export default function Set3() {
  ...
  useEffect(() => {
    async function init() {
      ...
      const { data: existing } = await supabase.from('set4_submissions').select('*').eq('username', user.username).single()
      if (existing) {
  ...
  useEffect(() => {
    async function handleMessage(e) {
      ...
      if (e.data.type === 'IELTS_SCORE_S4') {
        ...
        if (module === 'listening') {
          await supabase.from('set4_submissions').upsert({
  ...
  async function goListeningWarn() {
    setError('')
    const { data } = await supabase.from('set4_html').select('*').eq('id', 'listening').single()
  ...
  async function goReadingWarn() {
    ...
    const { data } = await supabase.from('set4_html').select('*').eq('id', 'reading').single()
  ...
  async function goWritingWarn() {
    ...
    const { data } = await supabase.from('set4_html').select('*').eq('id', 'writing').single()
  ...
  return (
    <>
      <Head><title>IELTS Set 4</title><meta name="viewport" content="width=device-width, initial-scale=1" /></Head>
      <style>{`
        ...
        <div style={{ maxWidth: 500, margin: '2rem auto', padding: '0 1rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
            <div>
              ...
              <div style={{ fontSize: 13, color: '#888', marginTop: 2 }}>Set 4 — Full Mock Test</div>
            </div>
      ...
      {screen === 'listening-warn' && (
        <div style={{ maxWidth: 460, margin: '3rem auto', padding: '0 1rem' }}>
          <div className="card" style={{ marginTop: 0, border: '1px solid #E24B4A' }}>
            <div className="logo" style={{ marginBottom: 10 }}>IELTS Set 4</div>
      ...
      {screen === 'done' && (
        <div style={{ maxWidth: 400, margin: '4rem auto', padding: '0 1rem', textAlign: 'center' }}>
          <div className="card" style={{ marginTop: 0 }}>
            ...
            <div style={{ fontSize: 18, fontWeight: 500 }}>Set 4 complete!</div>

      ...
      {screen === 'home' && (
        <div style={{ maxWidth: 540, margin: '2rem auto', padding: '0 1rem' }}>
          ...
          {availableSets.map((set, idx) => {
            const colors = ['#0F6E56', '#185FA5', '#9333EA', '#DC2626', '#EA580C']
            const bgColors = ['#E1F5EE', '#EFF6FF', '#F3E8FF', '#FEE2E2', '#FFEDD5']
            const color = colors[idx % colors.length]
            const bgColor = bgColors[idx % bgColors.length]
            return (
              <div key={set.set_number} className="card" style={{ border: `2px solid ${color}`, marginTop: 12 }}>
                <div style={{ fontWeight: 500, fontSize: 16, marginBottom: 6 }}>Set {set.set_number} — Full Mock Test</div>
                <div style={{ display: 'flex', gap: 6, marginBottom: 8, flexWrap: 'wrap' }}>
                  <div style={{ background: bgColor, color: color, padding: '3px 10px', borderRadius: 6, fontSize: 12, fontWeight: 500 }}>Listening</div>
                  <div style={{ color: '#888', fontSize: 12, display: 'flex', alignItems: 'center' }}>→</div>
                  <div style={{ background: bgColor, color: color, padding: '3px 10px', borderRadius: 6, fontSize: 12, fontWeight: 500 }}>Reading</div>
                  <div style={{ color: '#888', fontSize: 12, display: 'flex', alignItems: 'center' }}>→</div>
                  <div style={{ background: bgColor, color: color, padding: '3px 10px', borderRadius: 6, fontSize: 12, fontWeight: 500 }}>Writing</div>
                </div>
                ...
                <a href={`/set${set.set_number}`} style={{ display: 'inline-block', padding: '8px 18px', background: color, color: '#fff', borderRadius: 8, fontSize: 13, fontWeight: 500, textDecoration: 'none' }}>Go to Set {set.set_number} →</a>
              </div>
            )
          })}
          <div className="card">
